...
erpnext.selling.SalesForecast = frappe.ui.form.Controller.extend({
	...
	make_production_order: function() {
		...
		frappe.call({
			method: 'frappe.direction.customizations_2.get_actual_qty_for_sample',
			args: { doc: me.frm.doc },
			callback: function(r) {
				if (!r) return;
				...
				if (me.frm.doc.docstatus === 1) {
					me.frm.add_custom_button(
						...
						function() {
							if (!r.message) {
								frappe.msgprint(frappe._('Not enough quantity'));
								return;
							}

							frappe.model.open_mapped_doc({
								method: 'erpnext.manufacturing.doctype.sample_request.sample_request.make_production_order',
								frm: me.frm
							});
						},
						frappe._('Make')
