...
def get_location_details():
	result = frappe.db.sql("""select c.name as customer, c.customer_name, sp.employee, e.user_id from `tabCustomer` c
		...
		where e.user_id=%s and (isnull(c.latitude) or length(trim(c.latitude))=0 or isnull(c.longitude) or length(trim(c.longitude))=0)
		order by c.name
