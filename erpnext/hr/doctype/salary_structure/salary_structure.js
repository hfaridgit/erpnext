...
frappe.ui.form.on('Salary Structure', {
	...
	refresh: function(frm) {
		...
		frm.add_custom_button(__("Upgrade Insurance Values"),
			function() { 
				frappe.prompt([{fieldname:"percentage", fieldtype:"Percent", default:7, label: "Pecent of increase", reqd: 1}, 
				{fieldname:"base", fieldtype:"Check", label: "Base Value"}, 
				{fieldname:"variable", fieldtype:"Check", label: "Variable Value"}], 
				function(data) {
					$.each(frm.doc.employees || [], function(i, row) {
						if (data.base==1) {
							row.base = flt(row.base)+(flt(row.base)*flt(data.percentage)/100);
						}
						if (data.variable==1) {
							row.variable = flt(row.variable)+(flt(row.variable)*flt(data.percentage)/100);
						}
					});
					refresh_field("employees");
				}, __("Change Value"));
			}, "fa fa-sitemap", "btn-default");
