...
class Employee(NestedSet):
	...
	def allocate_leaves(self):
		if not self.date_of_joining:
			frappe.throw(_("Date of Joining not defined."))
		la = frappe.new_doc("Annual Leave Allocation")
		la.update({
	...
	def release_custody(self):
		custodies = frappe.db.sql("""select name, notify_user from `tabEmployee Custody` where status<>'Returned' and employee=%s""", (self.name), as_dict=1)
		for cs in custodies:
			frappe.db.sql("""delete from `tabToDo` where status='Open' and reference_type='Employee Custody' and reference_name=%s""", (cs.name))
			frappe.db.set_value("Employee Custody", cs.name, "status", "Pending Return")
			args = {
				'doctype': "Employee Custody",
				'name': cs.name,
				'assign_to': cs.notify_user,
				'description': "إخلاء طرف - رجاء عمل اللازم",
				'priority': "High",
				'notify': 1,
				'date': today()
			}
			assign_to.add(args)
		frappe.msgprint("تم إخطار جميع الأطراف لإخلاء الطرف")
...
def get_holiday_list_for_employee(employee, raise_exception=True):
	...
	holiday_list2 = ""
	if employee:
		holiday_list, shift = frappe.db.get_value("Employee", employee, ["holiday_list", "Shift"])
		if shift:
			holiday_list2 = frappe.db.get_value("Shifts", shift, "holiday_list")
		else:
			frappe.throw(_('Please set the Shift for Employee {0}').format(employee))
	else:
		company=frappe.db.get_value("Global Defaults", None, "default_company")
		holiday_list2 = frappe.db.get_value("Company", company, "default_holiday_list")

	if holiday_list:
		holiday_lists.append(holiday_list)

	if not holiday_list2 and raise_exception:
		frappe.throw(_('Please set a default Holiday List for Shift {0} or Company {1}').format(shift, company))
	if holiday_list2:
		holiday_lists.append(holiday_list2)
...
def is_holiday(employee, date=None):
	...
	if holiday_lists:
		hh = frappe.db.sql("""select holiday_date from `tabHoliday` where holiday_date=date and parent in %s""", (date, holiday_lists))
		if hh:
			return True
		else:
			return False
		#return frappe.get_all('Holiday List', dict(name in holiday_lists, holiday_date=date)) and True or False
