	...
	refresh: function() {
		...
		if (!cur_frm.doc.__islocal) {
			cur_frm.add_custom_button(__('Allocate Leaves'), function () {
				return frappe.call({
					doc: cur_frm.doc,
					method: 'allocate_leaves',
					callback: function() {
						cur_frm.refresh();
					}
				});
			});
			cur_frm.add_custom_button(__('Release Custody'), function () {
					frappe.confirm(
						__('Are you sure to start Custody release Action?'),
						function(){
							frappe.call({
										doc: cur_frm.doc,
										method: 'release_custody',
										callback: function() {
											//cur_frm.refresh();
										}
							});
		...
		cur_frm.add_custom_button(__('Show Employee History'), function() {
			frappe.set_route('List', 'Version',
				{ref_doctype: "Employee", docname: cur_frm.doc.docname});
		});
