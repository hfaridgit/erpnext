...
class PayrollEntry(Document):
	...
	def get_emp_list(self):
		...
		emp_list2 = frappe.db.sql("""select employee from `tabSalary Component Employee` 
						where docstatus!= 2 and parent = %(s_type)s""", {"s_type": self.salary_slip_type})
		if sal_struct:
			if emp_list2:
				cond += " and t1.name IN %(emp_list2)s "
			...
			%s """% cond, {"sal_struct": sal_struct, "emp_list2": emp_list2}, as_dict=True)
			return emp_list
	...
	def get_filter_condition(self):
		...
		cond = ''
		for f in ['company', 'business_unit', 'branch', 'department', 'designation']:
			if self.get(f):
	...
	def check_mandatory(self):
		for fieldname in ['company', 'business_unit', 'start_date', 'end_date']:
	...
	def create_salary_slips(self):
		...
		if emp_list:
			for emp in emp_list:
				if self.salary_component or not frappe.db.sql("""select
					...
					ss = frappe.get_doc({
						...
						"salary_component": self.salary_component if self.salary_component else None,
						"salary_slip_type": self.salary_slip_type,
	...
	def make_accural_jv_entry(self):
		...
		loan_details = self.get_loan_details() if not self.salary_component else None
		jv_name = ""
		...
		if earnings or deductions:
			...
			journal_entry.business_unit = self.business_unit
