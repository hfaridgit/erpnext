...
def get_gl_entries(filters):
	select_fields = """, sum(gle.debit_in_account_currency) as debit_in_account_currency,
		sum(gle.credit_in_account_currency) as credit_in_account_currency""" \
		if filters.get("show_in_account_currency") else ""

	group_by_condition = "group by gle.voucher_type, gle.voucher_no, gle.account, gle.cost_center" \
		if filters.get("group_by_voucher") else "group by gle.name"

	# @custom
	gl_entries = frappe.db.sql("""
		select
			gle.business_unit, gle.posting_date, gle.account, gle.party_type, gle.party,
...
def get_conditions(filters):
	...
	# @custom
	conditions.append("gle.voucher_type in ('Sales Invoice', 'Payment Entry')")
	...
	if filters.get("business_unit"):
		conditions.append("gle.business_unit=%(business_unit)s")
