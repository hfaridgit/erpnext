...
def _execute(filters=None, additional_table_columns=None, additional_query_columns=None):
	...
	for d in item_list:
		...
		row = [d.item_code, d.item_name, d.item_group, d.parent, d.posting_date, d.customer, d.customer_name, d.sales_person]
		...
		row += [
			...
			delivery_note, d.income_account, d.cost_center, d.qty, d.uom, d.stock_qty, d.stock_uom,
		]
...
def get_conditions(filters):
	...
	for opts in (("company", " and company=%(company)s"), ("business_unit", " and business_unit=%(business_unit)s"),
	...
	if filters.get("sales_person"):
		conditions += " and `tabSales Team`.sales_person = %(sales_person)s"
...
def get_items(filters, additional_query_columns):
	...
	return frappe.db.sql("""
		select
			...
			`tabSales Invoice`.update_stock, `tabSales Invoice Item`.uom,
			`tabSales Invoice Item`.qty, `tabSales Invoice Item`.uom,
			`tabSales Team`.sales_person {0}
		from `tabSales Invoice`
		left join `tabSales Invoice Item`
			on `tabSales Invoice`.name = `tabSales Invoice Item`.parent
		left join `tabSales Team`
			on `tabSales Team`.parent = `tabSales Invoice`.name
			and `tabSales Team`.idx = 1
