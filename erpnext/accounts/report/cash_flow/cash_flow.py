...
def execute(filters=None):
	...
	for cash_flow_account in cash_flow_accounts:
		...
		for account in cash_flow_account['account_types']:
			account_data = get_account_type_based_data(filters.company, filters.business_unit, 
...
def get_account_type_based_data(company, business_unit, account_type, period_list, accumulated_values):
	data = {}
	total = 0
	for period in period_list:
		...
		gl_sum = frappe.db.sql_list("""
			...
			from `tabGL Entry`
			where company=%s and business_unit=%s and posting_date >= %s and posting_date <= %s 
				and voucher_type != 'Period Closing Voucher'
				and account in ( SELECT name FROM tabAccount WHERE account_type = %s)
		""", (company, business_unit, start_date if accumulated_values else period['from_date'],
