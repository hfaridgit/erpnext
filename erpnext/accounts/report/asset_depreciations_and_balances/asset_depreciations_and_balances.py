...
def get_assets(filters):
	...
	if filters.business_unit:
		conditions = " and business_unit=" + filters.business_unit
		
	return frappe.db.sql("""
		select name, asset_category, purchase_date, gross_purchase_amount, disposal_date, status
		from `tabAsset` 
		where docstatus=1 and company=%s and purchase_date <= %s %s""", 
		(filters.company, filters.to_date, conditions), as_dict=1)
