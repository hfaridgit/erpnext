...
class ReceivablePayableReport(object):
	...
	def get_columns(self, party_naming_by, args):
		...
		columns += [_("Voucher Type") + "::110", _("Voucher No") + ":Dynamic Link/"+_("Voucher Type")+":120",
			_("Due Date") + ":Date:80", _("Sales Person") + ":Link/Sales Person:120"]
		...
		for gle in self.get_entries_till(self.filters.report_date, args.get("party_type")):
			if self.is_receivable_or_payable(gle, dr_or_cr, future_vouchers):
				...
				if abs(outstanding_amount) > 0.1/10**currency_precision:
					...
					sales_person = voucher_details.get(gle.voucher_no, {}).get("sales_person", "")

					row += [gle.voucher_type, gle.voucher_no, due_date, sales_person]
	...
	def get_voucher_details(self, party_type, dn_details):
		...
		if party_type == "Customer":
			for si in frappe.db.sql("""select si.name, si.due_date, si.po_no, st.sales_person
	...
	def prepare_conditions(self, party_type):
		...
		if self.filters.business_unit:
			conditions.append("business_unit=%s")
			values.append(self.filters.business_unit)
