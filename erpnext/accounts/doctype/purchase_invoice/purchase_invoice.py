...
class PurchaseInvoice(BuyingController):
	...
	def make_gl_entries(self, gl_entries=None, repost_future_gle=True, from_repost=False):
		...
		if gl_entries:
			gl_entries = self.distribute_gl_entries(gl_entries)
	...
	def distribute_gl_entries(self, gl_entries):
		bu = []
		group_item_amount = {}

		for item in self.items:
			if item.business_unit != self.business_unit:
				group_item_amount[item.business_unit] = group_item_amount.get(item.business_unit, 0) + item.amount
		for item in self.items:
			if item.business_unit in group_item_amount:
				x = frappe._dict({"business_unit": item.business_unit, "percent": flt(group_item_amount[item.business_unit] / self.total)})
				del group_item_amount[item.business_unit]
				bu.append(x)
		...
		if bu:
			...
			for d in gl_entries:
				if d.business_unit == "***":
					...
					tot2 = 0
					for item in bu:
						#frappe.msgprint(item.business_unit)
						cr_or_dr = "debit" if d.debit != 0 else "credit"
						dr_cr_amount = d.debit if d.debit != 0 else d.credit
						cr_or_dr2 = "debit_in_account_currency" if d.debit_in_account_currency != 0 else "credit_in_account_currency"
						dr_cr_amount2 = d.debit_in_account_currency if d.debit_in_account_currency != 0 else d.credit_in_account_currency
						tot += d.debit * item.percent if d.debit != 0 else d.credit * item.percent
						tot2 += d.debit_in_account_currency * item.percent if d.debit_in_account_currency != 0 else d.credit_in_account_currency * item.percent
						gl_m.append(
							self.get_gl_dict({
								"account": d.account,
								...
								cr_or_dr: flt(dr_cr_amount * item.percent), 
								"account_currency": d.account_currency,
								cr_or_dr2: flt(dr_cr_amount2 * item.percent), 
					...
					d.debit = d.debit - tot if d.debit != 0 else 0
					d.credit = d.credit - tot if d.credit != 0 else 0
					d.debit_in_account_currency = d.debit_in_account_currency - tot2 if d.debit_in_account_currency != 0 else 0
					d.credit_in_account_currency = d.credit_in_account_currency - tot2 if d.credit_in_account_currency != 0 else 0
					
			if gl_m:
				gl_entries.extend(gl_m)
		for d in gl_entries:
			if d.business_unit == "***":
				d.business_unit = self.business_unit
