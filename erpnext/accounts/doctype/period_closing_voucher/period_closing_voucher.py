...
class PeriodClosingVoucher(AccountsController):
	...
	def make_gl_entries(self):
		...
		for bu in frappe.db.sql("""select name as business_unit from `tabBusiness Unit` where company=%s""", self.company, as_dict=1):
			self.business_unit = bu.business_unit
			gl_entries = []
			net_pl_balance = 0
			pl_accounts = self.get_pl_balances()
	...
	def get_pl_balances(self):
		...
		return frappe.db.sql("""
			...
			and t2.docstatus < 2 and t2.company = %s and t1.business_unit = %s
			and t1.posting_date between %s and %s
			group by t1.account, t1.cost_center
		""", (self.company, self.business_unit, self.get("year_start_date"), self.posting_date), as_dict=1)
